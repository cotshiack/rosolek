...
enum BrothPreset: String, CaseIterable, Identifiable, Hashable {
    ...
    var defaultSelectedIDs: [String] {
        ...
        case .collagenPoultryReady:
            return ["szyje_kurczaka", "korpus_kurczaka", "lapki"]
        }
    }
...
enum BrothCalculator {
    ...
    private static func presetCalculation(
        ...
        case .collagenPoultryReady:
            return collagenPoultryPresetCalculation(
                pot: pot,
                request: request
            )
        }
    ...
    private static func collagenPoultryPresetCalculation(
        pot: Double,
        request: BrothCalculationRequest
    ) -> BrothCalculationResult {
        let waterLiters = interpolate(pot, table: [(5, 3.2), (7, 4.6), (10, 6.6), (12, 7.8)])
        let necks = roundedToTen(interpolate(pot, table: [(5, 560), (7, 800), (10, 1140), (12, 1360)]))
        let carcass = roundedToTen(interpolate(pot, table: [(5, 560), (7, 800), (10, 1140), (12, 1360)]))
        let feet = roundedToTen(interpolate(pot, table: [(5, 180), (7, 260), (10, 360), (12, 440)]))
        ...
        let parsley = roundedToFive(Double(totalVegetableGrams) * 0.22)
        let leek = max(0, totalVegetableGrams - carrot - celeriac - parsley)
        let onionCount = min(3, max(1, roundedToInt(waterLiters / 4.0)))
        let vegetableBreakdown = BrothVegetableBreakdown(totalGrams: totalVegetableGrams, carrotGrams: carrot, celeriacGrams: celeriac, parsleyRootGrams: parsley, leekGrams: leek, onionCount: onionCount)
        let vegetables = [
            VegetableAmount(name: "Marchew", amount: "\(carrot) g", note: nil),
            VegetableAmount(name: "Seler korzeń", amount: "\(celeriac) g", note: nil),
            VegetableAmount(name: "Pietruszka korzeń", amount: "\(parsley) g", note: nil),
            VegetableAmount(name: "Por", amount: "\(leek) g", note: nil),
            VegetableAmount(name: "Cebula opalana", amount: onionCount == 1 ? "1 szt." : "\(onionCount) szt.", note: nil)
        ]
        let estYield = roundedToOneDecimal(waterLiters * 0.78)
