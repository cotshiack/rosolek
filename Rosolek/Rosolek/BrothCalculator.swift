...
enum BrothCalculator {
    ...
    private static func collagenPoultryPresetCalculation(
        ...
        let baseEstimatedYield = roundedToOneDecimal(waterLiters * 0.78)
        let clarityAdjustedYield = adjustedYield(
            baseYieldLiters: baseEstimatedYield,
        ...
        let wingPieceLabel = "\(Int(wingPieces))"
        let thighPieceLabel = thighPieces == floor(thighPieces)
            ? "\(Int(thighPieces))"
            : thighPieces.formatted(.number.precision(.fractionLength(1)))
        var warnings: [String] = []
